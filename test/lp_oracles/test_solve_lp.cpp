...
void test_chebychev_cube(unsigned d, NT r) {
    ...
    auto res = ComputeChebychevBall<NT, Point>(A, b);
    CHECK(res.second == doctest::Approx(r));

    for (unsigned i = 0; i < d; ++i)
        CHECK(res.first[i] == doctest::Approx(0.0));
}
...
void test_chebychev_empty_cube(unsigned d) {
    ...
    CHECK(res.second == doctest::Approx(0.0));
}
...
void test_chebychev_infeasible_cube(unsigned d) {
    ...
    CHECK(res.second == doctest::Approx(-1.0));
}
...
void test_identical_intersection() {
    ...
    bool empty = false;
    Point p = PointInIntersection<VT>(V1, V1, direction, empty);
...
void test_corner_intersection() {
    ...
    bool empty = false;
    Point p = PointInIntersection<VT>(V1, V2, direction, empty);
...
void test_empty_intersection() {
    ...
    bool empty = false;
    PointInIntersection<VT>(V1, V2, direction, empty);
